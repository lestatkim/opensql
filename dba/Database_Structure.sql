...
/*	Name : Database Structure [ILS]
	Created / modify : 2017-03-10
	Author: Lestat Kim
	...
	,main as (
		...
		*/
		select
			...
			,ils_c.ordinal_position [Column_pos] ,t.name [Type]
		...
		from sys.columns _c, sys.types t, sys.tables _t
			...
			--(!) ils - your database
			 ,ils.information_schema.columns ils_c --Get tables only [ils] DB			
			 ,x --link query [x]
		where
			_t.object_id = _c.object_id
			and t.system_type_id = _c.system_type_id and t.user_type_id = _c.user_type_id
			and ils_c.column_name = _c.name and ils_c.table_name = _t.name
	)
